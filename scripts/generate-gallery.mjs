...
function build(){
  ...
  // simple categories
  for (const cat of ['fancy','mestor','rails','windows','fromShetah']){
    const catDir = path.join(imagesRoot, cat)
    if (fs.existsSync(catDir)){
      // Special handling for fromShetah: pair videos with posters (same basename.webp)
      if (cat === 'fromShetah'){
        const entries = listFiles(catDir).filter(d => d.isFile() && isMediaFile(d.name))
        const videos = entries.filter(d => videoExts.has(path.extname(d.name).toLowerCase()))
        const images = new Set(
          entries
            .filter(d => !videoExts.has(path.extname(d.name).toLowerCase()))
            .map(d => d.name)
        )
        const items = []
        for (const v of videos){
          const base = v.name.replace(/\.(mp4|webm)$/i, '')
          const posterName = `${base}.webp`
          const posterExists = images.has(posterName)
          items.push({ src: `/images/${cat}/${v.name}`, type: 'video', poster: posterExists ? `/images/${cat}/${posterName}` : undefined })
          if (posterExists) images.delete(posterName) // don't add poster as separate image
        }
        // Any remaining images that are not posters for videos will be added as images
        for (const imgName of images){
          items.push({ src: `/images/${cat}/${imgName}`, type: 'image' })
        }
        data[cat] = items
      } else {
        const files = listFiles(catDir)
          .filter(d => {
            if (!d.isFile() || !isMediaFile(d.name)) return false
            if (cat !== 'fancy') return true
            const ext = path.extname(d.name).toLowerCase()
            // In fancy: include .webp images and videos (.mp4/.webm)
            return ext === '.webp' || videoExts.has(ext)
          })
  ...
  fs.writeFileSync(path.join(outDir, 'windows.json'), JSON.stringify({ items: data.windows }, null, 2))
  fs.writeFileSync(path.join(outDir, 'fromShetah.json'), JSON.stringify({ items: data.fromShetah }, null, 2))
  // Do not overwrite pergulot.json here; it is generated by i18n script
  ...
  fs.writeFileSync(path.join(outDir, 'index.json'), JSON.stringify({
    categories: [
      ...
      { key: 'windows', count: data.windows.length },
      { key: 'fromShetah', count: data.fromShetah.length },
