...
Создайте кастомную политику `CDKDeployPolicy` (IAM → Policies → Create policy → JSON):
...
Прикрепите её к пользователю (Users → Add permissions → Attach policies directly).
