...
Сохраните ARN секрета. **Важно:** секрет должен быть в регионе **eu-north-1** (как и стек). Если создали в us-east-1 — создайте заново в eu-north-1.

### Шаг 2: IAM права

Пользователь/роль для CDK должен иметь права на: EC2 (DescribeVpcs, DescribeSubnets, CreateSecurityGroup), ECS, ECR, IAM, CloudFormation, ElasticLoadBalancing, Logs. Пользователь `pashkovsky-s3` (только S3) недостаточен — создайте отдельного пользователя с правами PowerUser или AdministratorAccess для деплоя.

### Шаг 3: Bootstrap CDK (один раз)
...
```bash
# Вариант A: по имени секрета (проще, без суффикса ARN)
npx cdk deploy ProfilesApiStack \
  -c secretsName=pashkovsky-profiles-api-secrets \
  --require-approval never

# Вариант B: по полному ARN (должен включать 6-символьный суффикс)
npx cdk deploy ProfilesApiStack \
  -c secretsArn=arn:aws:secretsmanager:eu-north-1:ACCOUNT:secret:pashkovsky-profiles-api-secrets-6YdziU \
  --require-approval never
