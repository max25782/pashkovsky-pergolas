  ...
  const columns = [
    {
      heading: t('product'),
      links: [
        { label: t('features_link'), href: '#features' },
        { label: t('pricing_link'), href: '#pricing' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 lg:py-16">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          {columns.map(({ heading, links }) => (
            <div key={heading}>
              <h4 className="font-syne font-700 text-sm text-white mb-4 uppercase tracking-wider">
                {heading}
              </h4>
        ...
        <div className="mt-12 pt-6 border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4">
          <p className="text-xs text-text-3">{t('copyright')}</p>
          <div className="flex items-center gap-1.5 text-xs text-text-3">
            <span>Built with</span>
            <span className="text-violet-400">♥</span>
            <span>for construction teams worldwide</span>
          </div>
        </div>
      </div>
    </footer>
