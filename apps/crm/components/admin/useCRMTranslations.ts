'use client'

import { useTranslations } from 'next-intl'

/**
 * Returns a typed translation object matching the old CRMTranslations shape.
 * Components use t.deals.title, t.common.save, etc. — same API as before.
 */
export function useCRMTranslations() {
  const common = useTranslations('common')
  const auth = useTranslations('auth')
  const nav = useTranslations('nav')
  const deals = useTranslations('deals')
  const leads = useTranslations('leads')
  const aiChats = useTranslations('aiChats')
  const articles = useTranslations('articles')
  const gallery = useTranslations('gallery')
  const orders = useTranslations('orders')
  const status = useTranslations('status')
  const workers = useTranslations('workers')

  return {
    common: {
      save: common('save'),
      saving: common('saving'),
      cancel: common('cancel'),
      delete: common('delete'),
      deleteConfirm: common('deleteConfirm'),
      close: common('close'),
      continue: common('continue'),
      logout: common('logout'),
      search: common('search'),
      loading: common('loading'),
      error: common('error'),
      noData: common('noData'),
      back: common('back'),
      next: common('next'),
      page: common('page'),
      of: common('of'),
      yes: common('yes'),
      no: common('no'),
      edit: common('edit'),
      phone: common('phone'),
      city: common('city'),
      address: common('address'),
    },
    auth: {
      enterAdminToken: auth('enterAdminToken'),
      adminTokenPlaceholder: auth('adminTokenPlaceholder'),
    },
    nav: {
      deals: nav('deals'),
      statistic: nav('statistic'),
      leads: nav('leads'),
      articles: nav('articles'),
      gallery: nav('gallery'),
      aiChats: nav('aiChats'),
      workers: nav('workers'),
      reports: nav('reports'),
    },
    deals: {
      title: deals('title'),
      newDeal: deals('newDeal'),
      statistics: deals('statistics'),
      searchPlaceholder: deals('searchPlaceholder'),
      customerName: deals('customerName'),
      customerPhone: deals('customerPhone'),
      customerEmail: deals('customerEmail'),
      customerCity: deals('customerCity'),
      projectType: deals('projectType'),
      stage: deals('stage'),
      width: deals('width'),
      depth: deals('depth'),
      shape: deals('shape'),
      material: deals('material'),
      colorRal: deals('colorRal'),
      price: deals('price'),
      myCost: deals('myCost'),
      orderDate: deals('orderDate'),
      materialOrderDate: deals('materialOrderDate'),
      materialReceivedDate: deals('materialReceivedDate'),
      installationDate: deals('installationDate'),
      lighting: deals('lighting'),
      manager: deals('manager'),
      notes: deals('notes'),
      sketch: deals('sketch'),
      openSketch: deals('openSketch'),
      btnCreateOffer2: deals('btnCreateOffer2'),
      offersTitle: deals('offersTitle'),
      loadingOffers: deals('loadingOffers'),
      shading: deals('shading'),
      finishType: deals('finishType'),
      selectOption: deals('selectOption'),
      ralPlaceholder: deals('ralPlaceholder'),
      woodOption: deals('woodOption'),
      laundryDetails: deals('laundryDetails'),
      laundryModel: deals('laundryModel'),
      laundryDistance: deals('laundryDistance'),
      laundryLight: deals('laundryLight'),
      profit: deals('profit'),
      calculating: deals('calculating'),
      revenue: deals('revenue'),
      noData: deals('noData'),
      materialCost: deals('materialCost'),
      laborCost: deals('laborCost'),
      totalCosts: deals('totalCosts'),
      costsPercent: deals('costsPercent'),
      netProfit: deals('netProfit'),
      profitMargin: deals('profitMargin'),
      kanbanEstimatedProfit: deals('kanbanEstimatedProfit'),
      materialOrdersSystemTotal: deals('materialOrdersSystemTotal'),
      materialOrdersOrderCount: (count: number) => deals('materialOrdersOrderCount', { count }),
      kanbanTapForDetails: deals('kanbanTapForDetails'),
      createdAt: deals('createdAt'),
      updatedAt: deals('updatedAt'),
      leadId: deals('leadId'),
      withoutName: deals('withoutName'),
      createDeal: deals('createDeal'),
      createNewDeal: deals('createNewDeal'),
      customerInfo: deals('customerInfo'),
      projectInfo: deals('projectInfo'),
      required: deals('required'),
      dealTitle: deals('dealTitle'),
      dealTitleCreate: deals('dealTitleCreate'),
      cm: deals('cm'),
      shekel: deals('shekel'),
      rectangle: deals('rectangle'),
      lShape: deals('lShape'),
      workType: deals('workType'),
      customerType: deals('customerType'),
      pricingModel: deals('pricingModel'),
      paymentPlan: deals('paymentPlan'),
      paymentPlanPreset: deals('paymentPlanPreset'),
      railingsDetails: deals('railingsDetails'),
      metersTotal: deals('metersTotal'),
      heightCm: deals('heightCm'),
      profileType: deals('profileType'),
      color: deals('color'),
      locationType: deals('locationType'),
      glassType: deals('glassType'),
      glazingSystem: deals('glazingSystem'),
      glazingAluminumGlass: deals('glazingAluminumGlass'),
      glazingWet: deals('glazingWet'),
      glazingDry: deals('glazingDry'),
      fenceDetails: deals('fenceDetails'),
      fenceVariant: deals('fenceVariant'),
      fenceClassic: deals('fenceClassic'),
      fenceHitech: deals('fenceHitech'),
      fenceHitechAngular: deals('fenceHitechAngular'),
      railingProfilePlaceholder: deals('railingProfilePlaceholder'),
      payments: deals('payments'),
      totalPaid: deals('totalPaid'),
      paidThisMonth: deals('paidThisMonth'),
      paidLastMonth: deals('paidLastMonth'),
      percentOfPrice: deals('percentOfPrice'),
      addPayment: deals('addPayment'),
      paidToDate: deals('paidToDate'),
      remaining: deals('remaining'),
      financeClientPrice: deals('financeClientPrice'),
      financeTotalCosts: deals('financeTotalCosts'),
      financeProfitLabel: deals('financeProfitLabel'),
      financeMarginLabel: deals('financeMarginLabel'),
      financeWarnNoCostsYet: deals('financeWarnNoCostsYet'),
      financeWarnAddPriceCosts: deals('financeWarnAddPriceCosts'),
      financeWarnZeroCostsNoMargin: deals('financeWarnZeroCostsNoMargin'),
      financePlaceholderPrice: deals('financePlaceholderPrice'),
      financePlaceholderCosts: deals('financePlaceholderCosts'),
      financeCostBreakdown: deals('financeCostBreakdown'),
      financeLaborFromShifts: deals('financeLaborFromShifts'),
      financeTotalInternalForProfit: deals('financeTotalInternalForProfit'),
      financeLoadingBreakdown: deals('financeLoadingBreakdown'),
      financeNoOrdersDash: deals('financeNoOrdersDash'),
      financeMaterialOrdersOrderCountTpl: (count: number) =>
        deals('financeMaterialOrdersOrderCountTpl', { count }),
      financeProfitFootnote: deals('financeProfitFootnote'),
      quickActions: deals('quickActions'),
      addExpense: deals('addExpense'),
      addWorkDay: deals('addWorkDay'),
      warrantyPdf: deals('warrantyPdf'),
      productLinesSection: deals('productLinesSection'),
      productLinesHint: deals('productLinesHint'),
      productLinesDerivedHint: deals('productLinesDerivedHint'),
      sectionClient: deals('sectionClient'),
      sectionProjectSpecs: deals('sectionProjectSpecs'),
      sectionDetailedProfit: deals('sectionDetailedProfit'),
      noExpensesYet: deals('noExpensesYet'),
      noWorkDaysYet: deals('noWorkDaysYet'),
      projectTypes: {
        pergola: deals('projectTypes.pergola'),
        railing: deals('projectTypes.railing'),
        gates: deals('projectTypes.gates'),
        windows: deals('projectTypes.windows'),
        laundry_closet: deals('projectTypes.laundry_closet'),
        fence: deals('projectTypes.fence'),
      },
      workTypes: {
        pergola: deals('workTypes.pergola'),
        railings: deals('workTypes.railings'),
        gates: deals('workTypes.gates'),
        facade: deals('workTypes.facade'),
        fence: deals('workTypes.fence'),
        other: deals('workTypes.other'),
      },
      customerTypes: {
        private: deals('customerTypes.private'),
        contractor: deals('customerTypes.contractor'),
      },
      pricingModels: {
        fixed: deals('pricingModels.fixed'),
        per_meter: deals('pricingModels.per_meter'),
        per_sqm: deals('pricingModels.per_sqm'),
        custom: deals('pricingModels.custom'),
      },
      locationTypes: {
        balcony: deals('locationTypes.balcony'),
        stairs: deals('locationTypes.stairs'),
        roof: deals('locationTypes.roof'),
        yard: deals('locationTypes.yard'),
        other: deals('locationTypes.other'),
      },
      stages: {
        new: deals('stages.new'),
        measure: deals('stages.measure'),
        offer: deals('stages.offer'),
        offer_approved: deals('stages.offer_approved'),
        material_ordered: deals('stages.material_ordered'),
        approved: deals('stages.approved'),
        production: deals('stages.production'),
        install: deals('stages.install'),
        done: deals('stages.done'),
      },
      filters: {
        allStages: deals('filters.allStages'),
        allTypes: deals('filters.allTypes'),
      },
      viewModes: {
        kanban: deals('viewModes.kanban'),
        table: deals('viewModes.table'),
      },
    },
    leads: {
      title: leads('title'),
      searchPlaceholder: leads('searchPlaceholder'),
      import: leads('import'),
      importSuccess: (count: number) => leads('importSuccess', { count }),
      importError: leads('importError'),
      name: leads('name'),
      phone: leads('phone'),
      email: leads('email'),
      city: leads('city'),
      source: leads('source'),
      status: leads('status'),
      notes: leads('notes'),
      leadTitle: leads('leadTitle'),
      notesPlaceholder: leads('notesPlaceholder'),
      createdAt: leads('createdAt'),
      lastMessage: leads('lastMessage'),
      page: leads('page'),
      whatsappGreeting: (name: string, company: string) =>
        leads('whatsappGreeting', { name, company }),
      whatsappGreetingNoName: (company: string) =>
        leads('whatsappGreetingNoName', { company }),
      groupByStatus: leads('groupByStatus'),
      groupBySource: leads('groupBySource'),
    },
    aiChats: {
      title: aiChats('title'),
      searchPlaceholder: aiChats('searchPlaceholder'),
      clientId: aiChats('clientId'),
      lastActivity: aiChats('lastActivity'),
      messages: aiChats('messages'),
      preview: aiChats('preview'),
      noMessages: aiChats('noMessages'),
      deleteDialog: aiChats('deleteDialog'),
      realtime: aiChats('realtime'),
      realtimeOff: aiChats('realtimeOff'),
      refresh: aiChats('refresh'),
      back: aiChats('back'),
      user: aiChats('user'),
      assistant: aiChats('assistant'),
    },
    articles: {
      title: articles('title'),
      createArticle: articles('createArticle'),
      editArticle: articles('editArticle'),
      deleteArticle: articles('deleteArticle'),
      deleteConfirm: articles('deleteConfirm'),
      titleLabel: articles('titleLabel'),
      contentLabel: articles('contentLabel'),
      slugLabel: articles('slugLabel'),
      publishedLabel: articles('publishedLabel'),
      save: articles('save'),
      cancel: articles('cancel'),
    },
    gallery: {
      title: gallery('title'),
      unavailable: gallery('unavailable'),
      mediaTitle: gallery('mediaTitle'),
      mediaSubtitle: gallery('mediaSubtitle'),
      search: gallery('search'),
      uncategorizedFirst: gallery('uncategorizedFirst'),
      importFromS3: gallery('importFromS3'),
      importing: gallery('importing'),
      refresh: gallery('refresh'),
      // ICU {count} — must not call gallery() without values at hook init
      selectedItems: (count: number) => gallery('selectedItems', { count }),
      clearSelection: gallery('clearSelection'),
      bulkCategoryLabel: gallery('bulkCategoryLabel'),
      applying: gallery('applying'),
      selectAll: (count: number) => gallery('selectAll', { count }),
      withCategory: (count: number) => gallery('withCategory', { count }),
      withoutCategory: (count: number) => gallery('withoutCategory', { count }),
      applyCategory: (count: number) => gallery('applyCategory', { count }),
      loadingImages: gallery('loadingImages'),
      noFiles: gallery('noFiles'),
      noFilesHint: gallery('noFilesHint'),
      catalogLabel: gallery('catalogLabel'),
      save: gallery('save'),
      editCategory: gallery('editCategory'),
      addCategory: gallery('addCategory'),
      loadMore: gallery('loadMore'),
      loading: gallery('loading'),
      noCategory: gallery('noCategory'),
      catPergolas: gallery('catPergolas'),
      catRailings: gallery('catRailings'),
      catFences: gallery('catFences'),
      catLaundryCover: gallery('catLaundryCover'),
    },
    orders: {
      title: orders('title'),
      orderNumber: orders('orderNumber'),
      customer: orders('customer'),
      status: orders('status'),
      totalWeight: orders('totalWeight'),
      totalAmount: orders('totalAmount'),
      finalAmount: orders('finalAmount'),
      createdAt: orders('createdAt'),
      items: orders('items'),
      notes: orders('notes'),
      deliveryDate: orders('deliveryDate'),
      paymentStatus: orders('paymentStatus'),
      discount: orders('discount'),
      save: orders('save'),
      cancel: orders('cancel'),
      generatePdf: orders('generatePdf'),
      colorLabel: orders('colorLabel'),
      colorPlaceholder: orders('colorPlaceholder'),
      statuses: {
        pending_price: orders('statuses.pending_price'),
        priced: orders('statuses.priced'),
        confirmed: orders('statuses.confirmed'),
        in_production: orders('statuses.in_production'),
        ready: orders('statuses.ready'),
        delivered: orders('statuses.delivered'),
        cancelled: orders('statuses.cancelled'),
      },
    },
    status: {
      loading: status('loading'),
      error: status('error'),
      noDeals: status('noDeals'),
      noLeads: status('noLeads'),
    },
    workers: {
      // title/subtitle/addWorker/etc. are read via useTranslations('workers') on pages;
      // do not resolve them here — strings with placeholders (e.g. {count}) would throw at hook init.
      colName: workers('colName'),
      colRole: workers('colRole'),
      colPhone: workers('colPhone'),
      colDailyRate: workers('colDailyRate'),
      colStatus: workers('colStatus'),
      colActions: workers('colActions'),
      active: workers('active'),
      inactive: workers('inactive'),
      editTitle: workers('editTitle'),
      addTitle: workers('addTitle'),
      firstName: workers('firstName'),
      lastName: workers('lastName'),
      phone: workers('phone'),
      role: workers('role'),
      dailyRate: workers('dailyRate'),
      isActive: workers('isActive'),
      requiredFields: workers('requiredFields'),
      dailyRatePositive: workers('dailyRatePositive'),
      saving: workers('saving'),
      timesheetDaysWorked: workers('timesheetDaysWorked'),
      timesheetHours: workers('timesheetHours'),
      timesheetLaborCost: workers('timesheetLaborCost'),
      timesheetLateExits: workers('timesheetLateExits'),
      timesheetHolidays: workers('timesheetHolidays'),
      timesheetHolidayPay: workers('timesheetHolidayPay'),
      timesheetDaysOff: workers('timesheetDaysOff'),
      timesheetTotalPayable: workers('timesheetTotalPayable'),
      shifts: workers('shifts'),
      showAllDays: workers('showAllDays'),
      exportPdf: workers('exportPdf'),
      exportingPdf: workers('exportingPdf'),
      exportPdfTitle: workers('exportPdfTitle'),
      addRecord: workers('addRecord'),
      noRecords: workers('noRecords'),
      colDate: workers('colDate'),
      colType: workers('colType'),
      colDeal: workers('colDeal'),
      colCity: workers('colCity'),
      colStart: workers('colStart'),
      colEnd: workers('colEnd'),
      colHours: workers('colHours'),
      colCost: workers('colCost'),
      shiftWork: workers('shiftWork'),
      shiftHoliday: workers('shiftHoliday'),
      shiftDayOff: workers('shiftDayOff'),
      shiftTypeLabel: workers('shiftTypeLabel'),
      unpaid: workers('unpaid'),
      addShiftTitle: workers('addShiftTitle'),
      workerLabel: workers('workerLabel'),
      loadingWorkers: workers('loadingWorkers'),
      selectWorker: workers('selectWorker'),
      dateLabel: workers('dateLabel'),
      startLabel: workers('startLabel'),
      endLabel: workers('endLabel'),
      notesLabel: workers('notesLabel'),
      perDay: workers('perDay'),
      workLog: workers('workLog'),
      addShift: workers('addShift'),
      loadingShifts: workers('loadingShifts'),
      noShifts: workers('noShifts'),
      worker: workers('worker'),
      workers_plural: workers('workers_plural'),
      deleteShiftConfirm: workers('deleteShiftConfirm'),
    },
  }
}
